...
<div class="card"><h2>Automation Status (V4.3 Professional)</h2>
<p>Auto Collect: <span id="status_collect">Idle</span> (see lead_collect_log)</p>
<p>Auto Send: <span id="status_send">Idle</span> (see auto_send_v40.log)</p>
<p>Pipeline: <span id="status_pipeline">Idle</span> (see pipeline.log)</p>
<p>Queue: <span id="queue_count">0</span></p>
<p>Sent: <span id="sent_count">0</span> / Failed: <span id="failed_count">0</span></p>
<p>Last Run: <span id="last_run">N/A</span></p>
</div>
...
<!-- V5 Professional Dashboard 动态显示 -->
<script>
function refreshStatus(){
    fetch('/automation_status_api')
    .then(response => response.json())
    .then(data => {
        document.getElementById('status_collect').innerText = data.collect_status;
        document.getElementById('status_send').innerText = data.send_status;
        document.getElementById('status_pipeline').innerText = data.pipeline_status;
        document.getElementById('queue_count').innerText = data.queue_count;
        document.getElementById('sent_count').innerText = data.sent_count;
        document.getElementById('failed_count').innerText = data.failed_count;
