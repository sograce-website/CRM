...
def home(request: Request, q: str = ""):
    ...
    role = USERS.get(username, {}).get("role", "sales")
    conn = db()
    c = conn.cursor()
    if q:
        like = f"%{q}%"
    ...
    for l in leads:
        ...
        <tr {highlight}>
            ...
            <td>{l[6]}</td><td>{l[7]}</td><td>{l[8]}</td><td>{l[14]}</td><td>{l[15]}</td><td>{l[16]}</td><td>${l[17] or '0'}</td>
            <td>
    ...
    return f"""
<html><head><title>SOGRACE CRM V2.6 Professional P3 Team Edition</title>
<style>
...
<div class="header">SOGRACE CRM V2.6 Professional P3 Team Edition <a style="float:right;font-size:16px" href="/logout">Logout</a></div>
<div class="container">
...
def quick_update(lead_id:int, level:str=Form("C"), owner:str=Form(""), next_followup:str=Form(""), status:str=Form("NEW"), note:str=Form(""), product_interest:str=Form(""), customer_value:str=Form("★"), expected_amount:str=Form("0")):
    conn=db()
