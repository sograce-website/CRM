...
def _safe_tail(path, max_chars=5000):
    try:
        p = FilePath(path)
        if not p.exists():
            return ""
...
def automation_metrics():
    ...
    queue_file = None
    for qf in ["/home/admin/crm/email_queue_v34.csv", "/home/admin/crm/email_queue.csv"]:
        if FilePath(qf).exists():
            queue_file = qf
            break

    queue_ready = queue_sent = queue_failed = queue_skipped = 0
    if queue_file:
        try:
            with open(queue_file, "r", encoding="utf-8-sig", errors="ignore", newline="") as f:
    ...
    send_log = _safe_tail("/home/admin/crm/auto_send_v40.log", 20000)
    collect_log = _safe_tail("/home/admin/crm/lead_collector.log", 20000)

    today_sent = 0
    for line in send_log.splitlines():
        if today in line and ("SENT:" in line or "Email Sent" in line):
            today_sent += 1

    last_auto_send = _file_mtime_text("/home/admin/crm/auto_send_v40.log")
    last_collect = _file_mtime_text("/home/admin/crm/lead_collector.log")

    conn = db()
    today_replies = conn.execute(
        "SELECT COUNT(*) FROM leads WHERE status='REPLIED' AND IFNULL(last_contact,'') LIKE ?",
        (today + "%",)
    ).fetchone()[0]
    total_replied = conn.execute("SELECT COUNT(*) FROM leads WHERE status='REPLIED'").fetchone()[0]
    conn.close()

    return {
        "queue_file": queue_file or "N/A",
        ...
        "last_auto_send": last_auto_send,
        "last_collect": last_collect,
        "auto_collect_status": "READY" if collect_log else "NO LOG",
        "auto_send_status": "READY" if send_log else "NO LOG",
    }

def recent_replies(limit=8):
    conn = db()
    rows = conn.execute("""
        SELECT id, company, email, status, last_contact
        FROM leads
        WHERE status='REPLIED' OR history LIKE '%Reply Received%'
        ORDER BY IFNULL(last_contact,'') DESC, id DESC
        LIMIT ?
    """, (limit,)).fetchall()
    conn.close()
    return rows

def source_ranking(limit=8):
    conn = db()
    rows = conn.execute("""
        SELECT COALESCE(NULLIF(source,''),'Unknown') AS s, COUNT(*)
        FROM leads
        GROUP BY s
        ORDER BY COUNT(*) DESC
        LIMIT ?
...
def render_v44_dashboard(stats_data, pipeline_value, quote_rate, order_rate):
    m = automation_metrics()
    replies = recent_replies()
    sources = source_ranking()

    stat_cards = ""
    for name in ["TOTAL","NEW","CONTACTED","REPLIED","QUOTED","SAMPLE","NEGOTIATION","ORDERED","LOST"]:
        stat_cards += f"<div class='v44-stat'><div class='v44-label'>{name}</div><div class='v44-num'>{stats_data.get(name,0)}</div></div>"

    automation_cards = f"""
    <div class='v44-stat'><div class='v44-label'>AUTO COLLECT</div><div class='v44-num small'>{m['auto_collect_status']}</div><div class='v44-sub'>Last: {m['last_collect']}</div></div>
    <div class='v44-stat'><div class='v44-label'>AUTO SEND</div><div class='v44-num small'>{m['auto_send_status']}</div><div class='v44-sub'>Last: {m['last_auto_send']}</div></div>
    <div class='v44-stat'><div class='v44-label'>QUEUE READY</div><div class='v44-num'>{m['queue_ready']}</div><div class='v44-sub'>Sent {m['queue_sent']} / Failed {m['queue_failed']}</div></div>
    <div class='v44-stat'><div class='v44-label'>TODAY SENT</div><div class='v44-num'>{m['today_sent']}</div></div>
    <div class='v44-stat'><div class='v44-label'>TODAY REPLIES</div><div class='v44-num'>{m['today_replies']}</div><div class='v44-sub'>Total replied {m['total_replied']}</div></div>
    <div class='v44-stat'><div class='v44-label'>PIPELINE VALUE</div><div class='v44-num small'>${pipeline_value:,.0f}</div><div class='v44-sub'>Quote {quote_rate}% / Order {order_rate}%</div></div>
    """

    if replies:
        reply_rows = "".join([
            f"<tr><td><a href='/lead/{r[0]}'>{r[1] or ''}</a></td><td>{r[2] or ''}</td><td>{r[3] or ''}</td><td>{r[4] or ''}</td></tr>"
            for r in replies
        ])
    else:
        reply_rows = "<tr><td colspan='4'>No customer replies yet. Run reply_center_v44.py to sync inbox.</td></tr>"

    source_rows = "".join([f"<tr><td>{s}</td><td>{c}</td></tr>" for s,c in sources]) or "<tr><td colspan='2'>No source data</td></tr>"

    return f"""
    <div class='v44-title'>CRM V4.4 Professional Dashboard</div>
    ...
    <div class='card'>
        ...
        <div class='v44-grid auto'>
            {automation_cards}
        </div>
        <p class='v44-note'>Queue file: {m['queue_file']}</p>
    </div>

    <div class='v44-two'>
        <div class='card'>
            <h2>Recent Customer Replies</h2>
            <table>
...
def home(request: Request, q: str = ""):
    ...
    v44_dashboard_html = render_v44_dashboard(stats_data, pipeline_value, quote_rate, order_rate)
...
.v44-title{{font-size:24px;font-weight:bold;margin-bottom:18px;color:#ffffff}}
.v44-grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:15px;margin-bottom:20px}}
.v44-grid.auto{{grid-template-columns:repeat(3,1fr)}}
.v44-stat{{background:#142848;padding:18px;border-radius:14px;box-shadow:0 0 0 1px rgba(255,255,255,.04)}}
.v44-label{{font-size:13px;color:#9fb5d1;letter-spacing:.5px}}
.v44-num{{font-size:30px;font-weight:bold;margin-top:8px;color:#ffffff}}
.v44-num.small{{font-size:20px}}
.v44-sub{{font-size:12px;color:#9fb5d1;margin-top:6px}}
.v44-two{{display:grid;grid-template-columns:2fr 1fr;gap:20px}}
.v44-note{{color:#9fb5d1;font-size:12px}}
</style></head><body>
...
<a class="export" href="/reply_center">Reply Center</a>
...
def reply_center(request: Request):
    ...
    for r in rows:
        rows_html += f"""
        <tr>
            <td><a href="/lead/{r[0]}">{r[1] or ''}</a></td>
            <td>{r[2] or ''}</td>
            <td>{r[3] or ''}</td>
            <td>{r[4] or ''}</td>
        </tr>
        """
    if not rows_html:
        rows_html = "<tr><td colspan='4'>No replied leads yet. Run reply_center_v44.py to sync inbox.</td></tr>"

    return HTMLResponse(f"""
    <html><head><title>Reply Center V4.4</title>
    <style>
    body{{font-family:Arial;background:#07111f;color:white;margin:0;padding:30px}}
    .card{{background:#111d33;padding:20px;border-radius:12px;margin-bottom:20px}}
    .grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:15px}}
    ...
    <h1>Reply Center V4.4</h1>
    <p><a class="btn" href="/">Back CRM</a></p>
    <div class="grid">
      <div class="stat">TOTAL<br>{stats_data.get('TOTAL',0)}</div>
      <div class="stat">REPLIED<br>{stats_data.get('REPLIED',0)}</div>
      <div class="stat">TODAY REPLIES<br>{m['today_replies']}</div>
      <div class="stat">QUEUE READY<br>{m['queue_ready']}</div>
    </div>
    <div class="card">
