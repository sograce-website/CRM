...
def _p5_clean_sender(raw):
    raw = _p5_decode_mime(raw or "")
    m = re.search(r"([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,})", raw)
    sender_email = m.group(1).lower() if m else ""
    sender = raw.replace("<"+sender_email+">", "").strip().strip('"') if sender_email else raw
    return sender, sender_email

def _p5_match_lead(sender_email):
    if not sender_email:
        return 0, ""
    conn = db()
    row = conn.execute("""
        SELECT id, COALESCE(owner,'')
        FROM leads
        ...
        if msg.is_multipart():
            for part in msg.walk():
                ...
                if ctype == "text/plain" and "attachment" not in disp:
                    ...
                    return (payload or b"").decode(charset, errors="ignore")[:8000]
            for part in msg.walk():
                if part.get_content_type() == "text/html":
                    payload = part.get_payload(decode=True)
                    charset = part.get_content_charset() or "utf-8"
                    html_body = (payload or b"").decode(charset, errors="ignore")
                    return re.sub("<[^<]+?>", " ", html_body)[:8000]
        else:
...
def _p5_sync_imap(limit=50):
    ...
    try:
        ...
        if typ != "OK":
            return {"synced": 0, "failed": 1, "message": "IMAP search failed"}
        ...
        for mid in ids:
            try:
                ...
                uid = mid.decode()
                subject = _p5_decode_mime(msg.get("Subject", ""))
                ...
                if lead_id:
                    ...
                    if line not in old_history:
                        conn.execute("UPDATE leads SET status='REPLIED', history=? WHERE id=?", (old_history + line, lead_id))
                synced += 1
            except Exception:
                failed += 1
        conn.commit()
...
def _um_db():
    return sqlite3.connect("/home/admin/crm/crm.db")
...
def _um_init():
    ...
    if "role" not in cols:
        c.execute("ALTER TABLE users ADD COLUMN role TEXT DEFAULT 'sales'")
...
def _um_admin_check(request):
    try:
        if 'is_login' in globals() and not is_login(request):
            return False
        username = request.cookies.get("crm_user", "")
        if 'get_user_role' in globals():
            return (get_user_role(username) or "").lower() == "admin"
        return True
    except:
        return True
...
def user_management(request: Request):
    if not _um_admin_check(request):
        return HTMLResponse("<h2>Access Denied</h2><p><a href='/'>Back</a></p>")

    rows = _um_rows()
    html_rows = ""
    for r in rows:
        uid = r["id"] if "id" in r.keys() else ""
        username = r["username"] if "username" in r.keys() else ""
        role = r["role"] if "role" in r.keys() else "sales"
        ...
        <tr>
            ...
            <td>{created or ''}</td>
            <td>
                <a class="btn" href="/user_enable/{uid}">Enable</a>
                <a class="btn warn" href="/user_disable/{uid}">Disable</a>
                <a class="btn danger" href="/user_delete/{uid}" onclick="return confirm('Delete this user?')">Delete</a>
            </td>
        </tr>
        """
    ...
    <html>
    <head>
    <title>User Management - SOGRACE CRM V4.5</title>
    <style>
    ...
    button,.btn{{background:#1683ff;color:white;padding:9px 14px;border-radius:6px;text-decoration:none;border:0;margin:3px;display:inline-block}}
    .warn{{background:#f0ad4e}}
    .danger{{background:#d93333}}
    ...
    <body>
    <h1>👥 SOGRACE CRM User Management V4.5</h1>
    <p><a href="/">← Back CRM</a></p>
...
@app.post("/user_add")
def user_add(request: Request, username: str = Form(...), password: str = Form(...), role: str = Form("sales"), status: str = Form("active")):
    if not _um_admin_check(request):
        return HTMLResponse("<h2>Access Denied</h2>")
    ...
    try:
        c.execute("INSERT INTO users(username,password,role,status,created_at) VALUES(?,?,?,?,?)", (username, password, role, status, now))
        conn.commit()
    except Exception as e:
        conn.close()
        return HTMLResponse(f"<h2>Error</h2><p>{e}</p><p><a href='/user_management'>Back</a></p>")
    conn.close()
    return RedirectResponse("/user_management", 303)

@app.get("/user_disable/{uid}")
def user_disable(uid: int):
    _um_init()
    conn = _um_db()
...
@app.get("/user_enable/{uid}")
def user_enable(uid: int):
    _um_init()
    conn = _um_db()
...
def sync_replies(request: Request):
    ...
    <p>Synced: {res.get('synced',0)}</p>
    <p>Failed: {res.get('failed',0)}</p>
    <p>{html.escape(str(res.get('message','')))}</p>
    <p><a href="/reply_center" style="color:white">Reply Center</a> | <a href="/" style="color:white">Dashboard</a></p>
    </body></html>
    """)
...
def reply_center_v52_p5(request: Request):
    ...
    for r in rows:
        ...
        <tr>
          ...
          <td>
            <a class="btn" href="/reply_done/{rid}">Done</a>
            <a class="btn orange" href="/reply_followup/{rid}">Follow Up</a>
          </td>
        </tr>
        """
    return HTMLResponse(f"""
...
@app.get("/reply_done/{rid}")
def reply_done(rid:int):
    conn = db()
    conn.execute("UPDATE email_replies SET status='DONE' WHERE id=?", (rid,))
    conn.commit()
    conn.close()
    return RedirectResponse("/reply_center", 303)

@app.get("/reply_followup/{rid}")
def reply_followup(rid:int):
    conn = db()
    row = conn.execute("SELECT lead_id FROM email_replies WHERE id=?", (rid,)).fetchone()
    tomorrow = (datetime.date.today() + datetime.timedelta(days=1)).isoformat()
    if row and row[0]:
        conn.execute("UPDATE leads SET next_followup=?, status='REPLIED' WHERE id=?", (tomorrow, row[0]))
    conn.execute("UPDATE email_replies SET status='FOLLOW_UP' WHERE id=?", (rid,))
    conn.commit()
    conn.close()
    return RedirectResponse("/reply_center", 303)
