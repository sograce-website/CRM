...
def home(q: str = ""):
    ...
    return f"""
<html><head><title>SOGRACE CRM V2.5 Professional P2</title>
<style>
...
<div class="header">SOGRACE CRM V2.5 Professional P2</div>
<div class="container">
...
def quick_update(lead_id:int, level:str=Form("C"), owner:str=Form(""), next_followup:str=Form(""), status:str=Form("NEW"), note:str=Form("")):
    conn=db()
    conn.execute("UPDATE leads SET level=?, owner=?, next_followup=?, status=?, note=? WHERE id=?", (level,owner,next_followup,status,note,lead_id))
    conn.commit()
    conn.close()
