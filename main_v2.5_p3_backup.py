...
def home(q: str = ""):
    ...
    return f"""
<html><head><title>SOGRACE CRM V2.5 Professional P3</title>
<style>
...
<div class="header">SOGRACE CRM V2.5 Professional P3</div>
<div class="container">
...
<table><tr><th>Company</th><th>Contact</th><th>Email</th><th>Country</th><th>WhatsApp</th><th>Category</th><th>Source</th><th>Quick Update</th><th>Action</th></tr>
{rows}</table></div>
</div></body></html>
"""
...
def quick_update(lead_id:int, level:str=Form("C"), owner:str=Form(""), next_followup:str=Form(""), status:str=Form("NEW"), note:str=Form("")):
    conn=db()
    ...
    if note:
        old_history += f"\\n{today} Note: {note}"
    conn.execute("UPDATE leads SET level=?, owner=?, next_followup=?, status=?, note=?, history=? WHERE id=?", (level,owner,next_followup,status,note,old_history,lead_id))
    conn.commit()
    conn.close()
