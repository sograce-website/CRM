...
def _auto_pipeline_worker(count: int):
    ...
    log_file = Path("/home/admin/crm/pipeline.log")
...
")
    ...
    log(f"PIPELINE START count={count}")
    ...
    try:
        ...
        try:
            ...
            log(f"COLLECT DONE added={added}")
        except Exception as e:
            log("COLLECT_READY_LEADS_ERROR: " + str(e))
            log(traceback.format_exc())

        try:
            from main import send_email_batch
            send_email_batch(count)
            log(f"SEND DONE count={count}")
        except Exception as e:
            log("SEND_EMAIL_BATCH_ERROR: " + str(e))
            log(traceback.format_exc())

    except Exception as e:
        log("PIPELINE_WORKER_FATAL_ERROR: " + str(e))
        log(traceback.format_exc())

    log(f"PIPELINE FINISHED count={count}")


@app.get("/auto_pipeline/{count}")
def auto_pipeline_web(count: int):
    background_tasks.add_task(_auto_pipeline_worker, count)
    return HTMLResponse(f"<h2>Pipeline {count} started</h2><p><a href='/'>Back to CRM</a></p>")
