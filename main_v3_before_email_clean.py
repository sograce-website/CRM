...
<button>Search</button><a class="export" href="/">Reset</a><a class="export" href="/export">Export CSV</a><a class="export" href="/bulk_test_5">Bulk Email Test 5</a></form>
...
"""
    ...
    for lead_id, company, email, history in rows:
        real_email = email.split(",")[0].strip()
        try:
            msg = MIMEText(message, "plain", "utf-8")
            msg["Subject"] = subject
            msg["From"] = SMTP_FROM
    ...
    return HTMLResponse(f"<h2>Bulk Email Test 5 Finished</h2><p>Sent: {sent}</p><p>Failed: {failed}</p>{report}<p><a href='/'>Back to CRM</a></p>")
